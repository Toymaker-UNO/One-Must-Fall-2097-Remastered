...
void projectile_finished(object *obj) {
    projectile_local *local = (projectile_local *)object_get_userdata(obj);
...
void projectile_debug(object *obj) {
    projectile_local *h = (projectile_local *)object_get_userdata(obj);
...
int projectile_create(object *obj, object *parent) {
    har *h = (har *)object_get_userdata(parent);
    // strore the HAR in local userdata instead
    projectile_local *local = (projectile_local *)calloc(1, sizeof(projectile_local));
    local->player_id = h->player_id;
    local->wall_bounce = 0;
    local->ground_freeze = 0;
    local->af_data = h->af_data;
    local->has_hit = false;
