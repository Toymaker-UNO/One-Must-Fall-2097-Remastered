...
void sprite_create(sprite *sp, void *src, int id) {
    ...
    // Load data
    sp->data = (surface *)omf_calloc(1, sizeof(surface));
    sp->owned = true;
    sd_vga_image raw;
...
sprite *sprite_copy(sprite *src) {
    ...
    sprite *copy = (sprite *)calloc(1, sizeof(sprite));
    copy->pos = src->pos;
    copy->id = src->id;

    // Copy surface
    copy->data = (surface *)omf_calloc(1, sizeof(surface));
    surface_create_from(copy->data, src->data);
    copy->owned = true;
    return copy;
}
