...
static void local_path_build(int path_id, const char *path, const char *ext) {
    if(str_ends_with_sep(path)) {
        ...
        local_paths[path_id] = (char *)omf_realloc(local_paths[path_id], len);
...
static void resource_path_build(int path_id, const char *path, const char *ext) {
    if(str_ends_with_sep(path)) {
        int len = strlen(path) + strlen(ext) + 1;
        resource_paths[path_id] = (char *)omf_realloc(resource_paths[path_id], len);
...
int pm_init(void) {
    ...
    char *state_base_dir;
    char *bin_base_dir;
    char *xdg_env, *resource_env, *shader_env;
    int m_ok = 0;
    ...
    resource_env = getenv("OPENOMF_RESOURCE_DIR");
    if(resource_env) {
        // make sure it ends with a separator
        local_path_build(RESOURCE_PATH, resource_env, "");
    }

    shader_env = getenv("OPENOMF_SHADER_DIR");
    if(shader_env) {
        // make sure it ends with a separator
