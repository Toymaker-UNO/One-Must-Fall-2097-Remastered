...
void controller_add_hook(controller *ctrl, controller *source, void (*fp)(controller *ctrl, int act_type)) {
    hook_function *h = (hook_function *)calloc(1, sizeof(hook_function));
    h->fp = fp;
    h->source = source;
...
static inline void ctrl_action_push(ctrl_event **ev, int action) {
    ctrl_event *ev_new = (ctrl_event *)calloc(1, sizeof(ctrl_event));

    ev_new->type = EVENT_TYPE_ACTION;
    ev_new->event_data.action = action;
...
void controller_cmd(controller *ctrl, int action, ctrl_event **ev) {
    ...
    if(ctrl->delay) {
        ...
        struct event_buffer_element *buf = (event_buffer_element *)vector_get(ctrl->buffer, (ctrl->gs->int_tick + ctrl->delay) % DELAY_BUFFER_SIZE);
        ...
        buf = (event_buffer_element *)vector_get(ctrl->buffer, (ctrl->gs->int_tick) % DELAY_BUFFER_SIZE);
...
void controller_close(controller *ctrl, ctrl_event **ev) {
    ...
    *ev = (ctrl_event *)omf_calloc(1, sizeof(ctrl_event));
...
bool controller_set_delay(controller *ctrl, uint8_t delay) {
    if(ctrl->supports_delay && delay > 0 && delay <= 10) {
        if(!ctrl->buffer) {
            ctrl->buffer = (vector *)omf_calloc(1, sizeof(vector));
