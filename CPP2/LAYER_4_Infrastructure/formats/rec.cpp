...
int sd_rec_load(sd_rec_file *rec, const char *file) {
    ...
    for(int i = 0; i < 2; i++) {
        ...
        if(sd_read_ubyte(r)) {
            rec->pilots[i].info.photo = (sd_sprite *)omf_calloc(1, sizeof(sd_sprite));
    ...
    rsize = sd_reader_filesize(r) - sd_reader_pos(r);
    rec->move_count = rsize / 7;
    rec->moves = (sd_rec_move *)omf_calloc(rec->move_count, sizeof(sd_rec_move));
    ...
    for(unsigned i = 0; i < rec->move_count; i++) {
        ...
        if(extra_length > 0) {
            ...
            if(action & 1) {
                rec->moves[i].action = (sd_action)(rec->moves[i].action | SD_ACT_PUNCH);
            }
            if(action & 2) {
                rec->moves[i].action = (sd_action)(rec->moves[i].action | SD_ACT_KICK);
            }
            switch(action & 0xF0) {
                case 16:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | SD_ACT_UP);
                    break;
                case 32:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | (SD_ACT_UP | SD_ACT_RIGHT));
                    break;
                case 48:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | SD_ACT_RIGHT);
                    break;
                case 64:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | (SD_ACT_DOWN | SD_ACT_RIGHT));
                    break;
                case 80:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | SD_ACT_DOWN);
                    break;
                case 96:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | (SD_ACT_DOWN | SD_ACT_LEFT));
                    break;
                case 112:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | SD_ACT_LEFT);
                    break;
                case 128:
                    rec->moves[i].action = (sd_action)(rec->moves[i].action | (SD_ACT_UP | SD_ACT_LEFT));
                    break;
            }
            ...
            if(unknown_len > 0) {
                rec->moves[i].extra_data = (char *)omf_calloc(unknown_len, 1);
    ...
    rec->moves = (sd_rec_move *)omf_realloc(rec->moves, rec->move_count * sizeof(sd_rec_move));
...
int sd_rec_insert_action(sd_rec_file *rec, unsigned int number, const sd_rec_move *move) {
    ...
    // Resize
    rec->moves = (sd_rec_move *)omf_realloc(rec->moves, (rec->move_count + 1) * sizeof(sd_rec_move));
