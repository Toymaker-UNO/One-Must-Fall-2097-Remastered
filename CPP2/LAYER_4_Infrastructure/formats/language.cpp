...
int sd_language_load(sd_language *language, const char *filename) {
    ...
    unsigned int *offsets = (unsigned int *)calloc(string_count + 1, sizeof(unsigned int));
    language->strings = (sd_lang_string *)omf_calloc(string_count, sizeof(sd_lang_string));
    ...
    for(unsigned i = 0; i < pos; i++) {
        ...
        language->strings[i].data = (char *)omf_calloc(len + 1, 1);
...
void sd_language_append(sd_language *language, const char *description, const char *data) {
    ...
    language->strings = (sd_lang_string *)omf_realloc(language->strings, language->count * sizeof(sd_lang_string));
