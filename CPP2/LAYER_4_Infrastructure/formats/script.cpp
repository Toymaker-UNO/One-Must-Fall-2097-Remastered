...
int sd_script_clear_tags(sd_script *script, int frame_id) {
    ...
    sd_script_frame *frame = (sd_script_frame *)vector_get(&script->frames, frame_id);
    if(frame == NULL) {
        return SD_INVALID_INPUT;
    }
...
int sd_script_get_tick_pos_at_frame(const sd_script *script, int frame_id) {
    ...
    for(int i = 0; i < frame_id; i++) {
        if((frame = (sd_script_frame *)vector_get(&script->frames, i)) != NULL) {
            len += frame->tick_len;
        }
    }
    return len;
...
static bool test_tag_slice(const str *test, sd_script_tag *out, str *src, int *now) {
    const int len = str_size(test);
    const int jmp = *now + len;
    if(sd_tag_info(str_c(test), &out->has_param, &out->key, &out->desc) == 0) {
        // Ensure that tag has no value, if value is not desired.
        if(!out->has_param && find_numeric_span(src, jmp) > jmp) {
            return false;
        }
...
static bool parse_tag(sd_script_tag *out, str *src, int *now) {
    ...
    for(int m = 3; m > 0; m--) {
        ...
        if(test_tag_slice(&test, out, src, now)) {
            str_free(&test);
            return true;
        }
        str_free(&test);
...
static bool parse_invalid_tag(sd_script_tag *out, str *src, int *now) {
    if(!is_tag_letter(str_at(src, *now))) {
        return false;
    }
...
const sd_script_frame *sd_script_get_frame(const sd_script *script, int frame_number) {
    ...
    return (const sd_script_frame *)vector_get(&script->frames, frame_number);
}
...
int sd_script_get_frame_index_at(const sd_script *script, unsigned ticks) {
    ...
    unsigned next, pos = 0;
    for(unsigned i = 0; i < vector_size(&script->frames); i++) {
        sd_script_frame *now = (sd_script_frame *)vector_get(&script->frames, i);
...
int sd_script_next_frame_with_sprite(const sd_script *script, int sprite_id, unsigned current_tick) {
    ...
    sd_script_frame *frame;
    for(unsigned i = 0; i < vector_size(&script->frames); i++) {
        frame = (sd_script_frame *)vector_get(&script->frames, i);
        next = pos + frame->tick_len;
...
int sd_script_set_tag(sd_script *script, int frame_id, const char *tag, int value) {
    ...
    // Get tag information
    sd_script_tag tag_val;
    if(sd_tag_info(tag, &tag_val.has_param, &tag_val.key, &tag_val.desc) != SD_SUCCESS) {
        return SD_INVALID_INPUT;
    }
    if(tag_val.has_param) {
        tag_val.value = value;
    }

    // Delete old tag (if exists), then add new.
    sd_script_delete_tag(script, frame_id, tag);
    vector_append(&frame->tags, &tag_val);
    return SD_SUCCESS;
}
