...
int sd_sounds_load(sd_sound_file *sf, const char *filename) {
    ...
    for(int i = 0; i <= data_block_count; i++) {
        ...
        if(sf->sounds[i].len > 0) {
            ...
            sf->sounds[i].data = (char *)omf_calloc(sf->sounds[i].len, 1);
...
int sd_sound_from_au(sd_sound_file *sf, int num, const char *filename) {
    int ret = SD_SUCCESS;
    uint32_t magic_number, data_start, data_size, data_type, data_freq, data_channels;
    size_t read_size = 0;
    ...
    // Header data
    data_start = sd_read_udword(r);
    data_size = sd_read_udword(r);
    data_type = sd_read_udword(r);
    data_freq = sd_read_udword(r);
    data_channels = sd_read_udword(r);
    ...
    sf->sounds[num].data = (char *)omf_calloc(read_size, 1);
