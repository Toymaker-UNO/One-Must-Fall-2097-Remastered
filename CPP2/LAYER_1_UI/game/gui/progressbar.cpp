...
void progressbar_set_progress(component *c, int percentage, bool animate) {
    progressbar *bar = (progressbar *)widget_get_obj(c);
...
static void progressbar_render(component *c) {
    ...
    if(bar->refresh || bar->display_percentage > bar->percentage) {
        ...
        if(w > 1 && h > 1) {
            ...
            if(bar->block == NULL) {
                bar->block = (surface *)omf_calloc(1, sizeof(surface));
            }
...
static void progressbar_layout(component *c, int x, int y, int w, int h) {
    ...
    // Allocate everything
    bar->background = (surface *)omf_calloc(1, sizeof(surface));
    bar->background_alt = (surface *)omf_calloc(1, sizeof(surface));
    bar->block = NULL;
...
component *progressbar_create(progressbar_theme theme, int orientation, int percentage) {
    ...
    progressbar *local = (progressbar *)calloc(1, sizeof(progressbar));
    local->theme = theme;
