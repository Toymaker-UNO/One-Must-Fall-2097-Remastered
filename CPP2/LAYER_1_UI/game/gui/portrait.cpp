...
static void portrait_render(component *c) {
    portrait *g = (portrait *)widget_get_obj(c);
    if(g->img != NULL) {
...
int portrait_load(sd_sprite *s, vga_palette *pal, int pic_id, int pilot_id) {
    ...
    if(ret != SD_SUCCESS) {
        log_error("Could not load PIC file %s: %s", filename, sd_get_error((SD_ERRORCODE)ret));
        return ret;
    } else {
...
void portrait_select(component *c, int pic_id, int pilot_id) {
    portrait *local = (portrait *)widget_get_obj(c);
    ...
    local->img = (sprite *)omf_calloc(1, sizeof(sprite));
    sd_sprite spr;
...
component *portrait_create(int pic_id, int pilot_id) {
    ...
    // Local information
    portrait *local = (portrait *)calloc(1, sizeof(portrait));
    local->max = 0;
    local->selected = 0;
    local->pic_id = -1;
