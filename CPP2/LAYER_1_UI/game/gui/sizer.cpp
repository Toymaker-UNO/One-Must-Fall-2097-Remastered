...
component *sizer_get(const component *nc, int item) {
    ...
    sizer *local = (sizer *)component_get_obj(nc);
    component **c;
    c = (component **)vector_get(&local->objs, item);
    if(c != NULL) {
        return *c;
    }
    return NULL;
...
float sizer_get_opacity(const component *c) {
    ...
    sizer *local = (sizer *)component_get_obj(c);
...
component *sizer_create(void) {
    ...
    sizer *local = (sizer *)calloc(1, sizeof(sizer));
