...
int is_key_bound(int key) {
    ...
    const char *compare = SDL_GetScancodeName((SDL_Scancode)key);
...
void menu_presskey_free(component *c) {
    presskey_menu_local *local = (presskey_menu_local *)menu_get_userdata(c);
    omf_free(local);
...
void menu_presskey_tick(component *c) {
    ...
    for(int i = 0; i < keys; i++) {
        ...
        if(state[i]) {
            if(is_key_bound(i) && strcmp(SDL_GetScancodeName((SDL_Scancode)i), *(local->key)) != 0) {
                ...
                omf_free(*(local->key));
                *(local->key) = omf_strdup(SDL_GetScancodeName((SDL_Scancode)i));
                m->finished = 1;
                return;
            }
        }
...
component *menu_presskey_create(char **key) {
    presskey_menu_local *local = (presskey_menu_local *)calloc(1, sizeof(presskey_menu_local));
