...
void resolution_toggled(component *c, void *userdata, int pos) {
    video_menu_data *local = (video_menu_data *)userdata;
...
void menu_video_done(component *c, void *u) {
    scene *s = (scene *)u;
    video_menu_data *local = (video_menu_data *)menu_get_userdata(c->parent);
    settings_video *v = &settings_get()->video;
...
void menu_video_free(component *c) {
    video_menu_data *local = (video_menu_data *)menu_get_userdata(c);
    omf_free(local);
...
component *menu_video_create(scene *s) {
    // Menu userdata
    video_menu_data *local = (video_menu_data *)calloc(1, sizeof(video_menu_data));
