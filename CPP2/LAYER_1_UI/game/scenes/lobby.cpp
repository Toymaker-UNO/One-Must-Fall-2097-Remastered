...
void lobby_free(scene *scene) {
    lobby_local *local = (lobby_local *)scene_get_userdata(scene);
...
void lobby_show_dialog(scene *scene, int style_id, char *dialog_text, dialog_clicked_cb callback) {
    lobby_local *local = (lobby_local *)scene_get_userdata(scene);
    if(local->dialog) {
        dialog_free(local->dialog);
        omf_free(local->dialog);
    }
    local->dialog = (dialog *)omf_calloc(1, sizeof(dialog));
    dialog_create(local->dialog, (dialog_style)style_id, dialog_text, 72, 60);
...
static void update_active_user_text(lobby_local *local) {
    ...
    lobby_user *user = (lobby_user *)list_get(&local->users, local->active_user);
...
void lobby_render_overlay(scene *scene) {
    ...
    if(local->mode > LOBBY_YELL) {
        ...
        list_iter_begin(&local->users, &it);
        int i = 0;
        while((user = (lobby_user *)iter_next(&it)) && i < 8) {
        ...
        log_event *log_msg;
        while((log_msg = (log_event *)iter_prev(&it)) && left > 0) {
...
void lobby_dialog_cancel_challenge(dialog *dlg, dialog_result result) {
    dialog_show(dlg, 0);
    scene *s = (scene *)dlg->userdata;
    lobby_local *local = (lobby_local *)scene_get_userdata(s);
    serial ser;
    serial_create(&ser);
...
void lobby_cancel_challenge(component *c, void *userdata) {
    menu *m = (menu *)sizer_get_obj(c->parent);
    scene *s = (scene *)userdata;
    lobby_local *local = (lobby_local *)scene_get_userdata(s);
    m->finished = 1;
...
void lobby_entered_name(component *c, void *userdata) {
    if(strlen(textinput_value(c))) {
        ...
        local->joinmenu = (menu *)sizer_get_obj(c->parent);
    }
}
...
void lobby_tick(scene *scene, int paused) {
    ...
    if(!local->nat) {
        local->nat = (nat_ctx *)omf_calloc(1, sizeof(nat_ctx));
        ...
        switch(event.type) {
            ...
            case ENET_EVENT_TYPE_CONNECT:
                ...
                if(local->opponent_peer && event.peer->address.host == local->opponent->address.host) {
                    ...
                    net_ctrl = (controller *)omf_calloc(1, sizeof(controller));
                    ...
                    case PACKET_SPECTATE: {
                        switch(control_byte & 0xf) {
                            case SPECTATE_ACCEPT: {
                                ...
                                c1 = (controller *)omf_calloc(1, sizeof(controller));
                                c2 = (controller *)omf_calloc(1, sizeof(controller));
...
int lobby_create(scene *scene) {
    ...
    local = (lobby_local *)omf_calloc(1, sizeof(lobby_local));
    scene_set_userdata(scene, local);
    ...
    if(game_state_get_player(scene->gs, 0)->ctrl->type == CTRL_TYPE_NETWORK) {
        ...
        local->nat = (nat_ctx *)local->peer->data;
