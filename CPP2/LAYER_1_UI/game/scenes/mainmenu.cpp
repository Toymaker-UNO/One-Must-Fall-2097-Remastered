...
void mainmenu_free(scene *scene) {
    mainmenu_local *local = (mainmenu_local *)scene_get_userdata(scene);
...
int mainmenu_create(scene *scene) {
    // Init local data
    mainmenu_local *local = (mainmenu_local *)calloc(1, sizeof(mainmenu_local));
    scene_set_userdata(scene, local);
    ...
    if(!player2->pilot) {
        player2->pilot = (sd_pilot *)omf_calloc(1, sizeof(sd_pilot));
