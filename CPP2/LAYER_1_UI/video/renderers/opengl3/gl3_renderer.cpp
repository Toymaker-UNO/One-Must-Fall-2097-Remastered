...
static bool setup_context(void *userdata, int window_w, int window_h, bool fullscreen, bool vsync, int aspect,
                          int framerate_limit) {
    gl3_context *ctx = (gl3_context *)userdata;
    GLuint pal_ubo_id;
    ...
    pal_ubo_id = shared_get_block(ctx->shared);
...
static void capture_screenshot(void *userdata) {
    ...
    unsigned char *buffer = (unsigned char *)malloc(r.w * r.h * 3);
...
static void render_area_finish(void *userdata, surface *dst) {
    ...
    unsigned char *buffer = (unsigned char *)malloc(r->w * r->h);
...
static void renderer_create(renderer *gl3_renderer) {
    gl3_renderer->ctx = (gl3_context *)omf_calloc(1, sizeof(gl3_context));
}
