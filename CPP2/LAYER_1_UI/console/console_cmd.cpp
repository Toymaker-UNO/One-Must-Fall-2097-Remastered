...
int sort_command_by_name(const void *a, const void *b) {
    hashmap_pair *const *h1 = (hashmap_pair *const *)a;
    hashmap_pair *const *h2 = (hashmap_pair *const *)b;
    return strcmp((const char *)(*h1)->key, (const char *)(*h2)->key);
}
...
int console_cmd_help(game_state *gs, int argc, char **argv) {
    ...
    foreach(it, ppair) {
        char *name = (char *)(*ppair)->key;
        command *cmd = (command *)(*ppair)->value;
...
int console_cmd_music(game_state *gs, int argc, char **argv) {
    if(argc == 2) {
        int i;
        if(strtoint(argv[1], &i)) {
            audio_play_music((resource_id)(PSM_END + i));
        }
    }
    return 0;
}
