...
static void console_handle_line(game_state *gs) {
    ...
    if(str_size(&con->input) == 0) {
        ...
        if(argc > 0) {
            char **argv = (char **)omf_calloc(argc, sizeof(char *));
            void *val = 0;
            unsigned int len;
...
bool console_init(void) {
    if(con != NULL)
        return false;
    con = (console *)omf_calloc(1, sizeof(console));
    con->is_open = false;
    ...
    if(e->type == SDL_TEXTINPUT) {
        ...
        if(scancode == SDL_SCANCODE_UP) {
            if(con->hist_pos < HISTORY_MAX && con->hist_pos < (signed int)(list_size(&con->history) - 1)) {
                con->hist_pos++;
                str_set_c(&con->input, (const char *)list_get(&con->history, con->hist_pos));
            }
