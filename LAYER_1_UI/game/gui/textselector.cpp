...
void textselector_clear_options(component *c) {
    text_selector *t = (text_selector *)widget_get_obj(c);
    vector_clear(&t->options);
}

void textselector_add_option(component *c, const char *value) {
    text_selector *tb = (text_selector *)widget_get_obj(c);
    char *opt_val = omf_strdup(value);
    vector_append(&tb->options, &opt_val);
}

const char *textselector_get_current_text(const component *c) {
    text_selector *tb = (text_selector *)widget_get_obj(c);
    char **text = (char **)vector_get(&tb->options, *tb->pos);
    if(text != NULL) {
        return *text;
    }
    return NULL;
...
static void refresh(component *c) {
    ...
    if(vector_size(&t->options) > 0 && str_size(&t->title) > 0) {
        // label & options
        char **opt = (char **)vector_get(&t->options, *t->pos);
        if(opt) {
            str_from_format(&formatted, "%s %s", str_c(&t->title), *opt);
        } else {
            str_from_format(&formatted, "%s NULL", str_c(&t->title));
        }
    } else if(vector_size(&t->options) > 0) {
        // no label, just options
        char **opt = (char **)vector_get(&t->options, *t->pos);
        str_from_format(&formatted, "%s", *opt);
    } else {
        // no options, just label
        str_from_format(&formatted, "%s -", str_c(&t->title));
    }
    text_set_from_str(t->text, &formatted);
    str_free(&formatted);
}
...
component *textselector_create(const char *text, const char *help, textselector_toggle_cb cb, void *userdata) {
    ...
    text_selector *t = (text_selector *)calloc(1, sizeof(text_selector));
...
component *textselector_create_bind(const char *text, const char *help, textselector_toggle_cb toggle_cb,
    ...
    text_selector *ts = (text_selector *)widget_get_obj(c);
