...
static void trnselect_render(component *c) {
    trnselect *g = (trnselect *)widget_get_obj(c);
...
void trnselect_next(component *c) {
    trnselect *local = (trnselect *)widget_get_obj(c);
    local->selected++;
...
sd_tournament_file *trnselect_selected(component *c) {
    trnselect *local = (trnselect *)widget_get_obj(c);
    return (sd_tournament_file *)vector_get(&local->tournaments, local->selected);
}
...
component *trnselect_create(void) {
    ...
    // Local information
    trnselect *local = (trnselect *)calloc(1, sizeof(trnselect));
    local->selected = 0;
    local->img = NULL;
    local->label = NULL;
