...
static void cutscene_input_tick(scene *scene) {
    cutscene_local *local = (cutscene_local *)scene_get_userdata(scene);
    game_player *player1 = game_state_get_player(scene->gs, 0);
            ...
            if(i->type == EVENT_TYPE_ACTION) {
                if(i->event_data.action == ACT_KICK || i->event_data.action == ACT_PUNCH) {
                    if(player1->chr && player1->chr->cutscene_text[local->pos + 1]) {
                        ...
                        local->pos++;
                        text_set_from_str(local->current, (const str *)vector_get(&local->texts, local->pos));
                    } else {
...
int cutscene_create(scene *scene) {
    cutscene_local *local = (cutscene_local *)calloc(1, sizeof(cutscene_local));
    ...
    switch(scene->id) {
        ...
        case SCENE_END1: {
            ...
            obj = (object *)omf_calloc(1, sizeof(object));
