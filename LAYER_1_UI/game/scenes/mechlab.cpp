...
bool mechlab_find_last_player(scene *scene) {
    mechlab_local *local = (mechlab_local *)scene_get_userdata(scene);
    ...
    if(!p1->chr && last_name != NULL) {
        sd_chr_file *chr = (sd_chr_file *)calloc(1, sizeof(sd_chr_file));
        int ret = sg_load(chr, last_name);
        if(ret != SD_SUCCESS) {
            omf_free(chr);
            log_error("Could not load saved game for playername '%s': %s!", last_name, sd_get_error((SD_ERRORCODE)ret));
            last_name = NULL;
        } else {
            p1->chr = chr;
    ...
    if(p1->chr == NULL) {
        ...
        object_free(local->mech);
        omf_free(local->mech);
        local->mech = (object *)omf_calloc(1, sizeof(object));
...
void mechlab_tick(scene *scene, int paused) {
    ...
    if(trnmenu_is_finished(root)) {
        ...
        if(local->dashtype == DASHBOARD_NEW_PLAYER) {
            ...
            player1->chr = (sd_chr_file *)omf_calloc(1, sizeof(sd_chr_file));
        ...
        case DASHBOARD_NEW_PLAYER: {
            if(player1->chr) {
                if(&player1->chr->pilot == player1->pilot) {
                    player1->pilot = (sd_pilot *)omf_calloc(1, sizeof(sd_pilot));
                }
                sd_chr_free(player1->chr);
                omf_free(player1->chr);
            }
...
int mechlab_create(scene *scene) {
    // Alloc
    mechlab_local *local = (mechlab_local *)calloc(1, sizeof(mechlab_local));
    local->selling = false;
