...
void melee_free(scene *scene) {
    melee_local *local = (melee_local *)scene_get_userdata(scene);
...
int melee_event_cb(scene *scene, SDL_Event *e) {
    ...
    if(local->page == HAR_SELECT && e->type == SDL_KEYDOWN && SDLK_1 <= e->key.keysym.sym &&
       e->key.keysym.sym <= SDLK_6) {
        ...
        game_player *player = game_state_get_player(scene->gs, player_id);
        uint8_t color = sd_pilot_get_player_color(player->pilot, (player_color)pal_id);
        color = (color + 1) % 16;
        sd_pilot_set_player_color(player->pilot, (player_color)pal_id, color);
        palette_load_player_colors(&player->pilot->palette, player_id);
    }
    return 1;
}
...
int melee_create(scene *scene) {
    // Init local data
    melee_local *local = (melee_local *)calloc(1, sizeof(melee_local));
    scene_set_userdata(scene, local);
    ...
    unsigned char *black = (unsigned char *)calloc(1, 51 * 36);
