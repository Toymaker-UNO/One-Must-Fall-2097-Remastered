...
void game_menu_quit(component *c, void *userdata) {
    ...
    arena_local *local = (arena_local *)scene_get_userdata((scene *)userdata);
    local->winner = 1;
...
void scene_fight_anim_done(void *scenedata, void *userdata) {
    ...
    // object *parent = game_state_find_object(s->gs, parent_id;
    arena_local *arena = (arena_local *)scene_get_userdata(s);
...
void scene_fight_anim_start(void *scenedata, void *userdata) {
    ...
    scene *sc = (scene *)scenedata;
    game_state *gs = sc->gs;
    scene *s = game_state_get_scene(gs);
    animation *fight_ani = &bk_get_info(s->bk_data, 10)->ani;
    object *fight = (object *)calloc(1, sizeof(object));
    object_create(fight, gs, fight_ani->start_pos, vec2f_create(0, 0));
    object_set_stl(fight, bk_get_stl(s->bk_data));
    ...
    ticktimer_add(&s->tick_timer, 24, scene_fight_anim_done, NULL /*fight->id*/);
}
...
void scene_youwin_anim_done(object *parent) {
    ...
    // mark object as finished manually, if necessary.
    parent->animation_state.finished = 1;
    arena_local *local = (arena_local *)scene_get_userdata(parent->gs->sc);
    local->win_state = DONE;
}

void scene_youwin_anim_start(void *userdata) {
    // Start FIGHT animation
    game_state *gs = (game_state *)userdata;
    scene *s = game_state_get_scene(gs);
    animation *youwin_ani = &bk_get_info(s->bk_data, 9)->ani;
    object *youwin = (object *)calloc(1, sizeof(object));
    object_create(youwin, gs, youwin_ani->start_pos, vec2f_create(0, 0));
    object_set_stl(youwin, bk_get_stl(s->bk_data));
...
void scene_youlose_anim_start(void *userdata) {
    ...
    game_state *gs = (game_state *)userdata;
    scene *s = game_state_get_scene(gs);
    animation *youlose_ani = &bk_get_info(s->bk_data, 8)->ani;
    object *youlose = (object *)calloc(1, sizeof(object));
    object_create(youlose, gs, youlose_ani->start_pos, vec2f_create(0, 0));
    object_set_stl(youlose, bk_get_stl(s->bk_data));
...
void arena_screengrab_winner(scene *sc) {
    ...
    har *h1 = (har *)object_get_userdata(o1);
...
static void arena_end(scene *sc) {
    game_state *gs = sc->gs;
    arena_local *local = (arena_local *)scene_get_userdata(sc);
    const scene *s = game_state_get_scene(gs);
    fight_stats *fight_stats = &gs->fight_stats;

    int winner_player_id = local->winner;
    int loser_player_id = !winner_player_id;
    game_player *player_winner = game_state_get_player(s->gs, winner_player_id);
    game_player *player_loser = game_state_get_player(s->gs, loser_player_id);
    object *winner = game_state_find_object(s->gs, game_player_get_har_obj_id(player_winner));
    har *winner_har = (har *)object_get_userdata(winner);
    ...
    if(s->gs->init_flags->playback == 1) {
        ...
        har *p1_har = (har *)object_get_userdata(game_state_find_object(gs, game_player_get_har_obj_id(p1)));
        har *p2_har = (har *)object_get_userdata(game_state_find_object(gs, game_player_get_har_obj_id(p2)));

        // Convert screen captures to grayscale
        har_screencaps *caps = &(fight_stats->winner == 0 ? p1 : p2)->screencaps;
        vga_palette *pal = bk_get_palette(s->bk_data, 0);
        ...
        if(game_state_get_player(s->gs, 0)->ctrl->type == CTRL_TYPE_NETWORK) {
            net_controller_set_winner(game_state_get_player(s->gs, 0)->ctrl, local->winner);
        }
        if(game_state_get_player(s->gs, 1)->ctrl->type == CTRL_TYPE_NETWORK) {
            net_controller_set_winner(game_state_get_player(s->gs, 1)->ctrl, local->winner);
        }
    ...
    if(is_singleplayer(s->gs) && winner_player_id == 0) {
...
void arena_reset(scene *sc) {
    ...
    if(local->rounds == 1) {
        ...
        object *ready = (object *)calloc(1, sizeof(object));
        ...
        object *round = (object *)calloc(1, sizeof(object));
        ...
        object *number = (object *)calloc(1, sizeof(object));
...
bool can_wallslam(int player_id, scene *scene) {
    ...
    har *h = (har *)object_get_userdata(o_har);
    object *o_har2 =
        game_state_find_object(scene->gs, game_player_get_har_obj_id(game_state_get_player(scene->gs, !player_id)));
    ...
    if(abs_velocity_h > 2) {
        ...
        if((abs_velocity_h + 0.5f) > tolerance && (h->state == STATE_RECOIL || h->state == STATE_DEFEAT)) {
            ...
            for(int i = 0; i < amount; i++) {
                ...
                object *dust = (object *)calloc(1, sizeof(object));
...
void arena_har_defeat_hook(int loser_player_id, scene *scene) {
    ...
    arena_local *local = (arena_local *)scene_get_userdata(scene);
    ...
    if(score->rounds >= ceilf(local->rounds / 2.0f)) {
        ...
        if(is_singleplayer(gs)) {
            ...
            if(player_loser->pilot->pilot_id == PILOT_KREISSACK && player_loser->pilot->har_id == HAR_NOVA) {
                har *loser_har = (har *)object_get_userdata(loser);
...
void arena_har_hook(har_event event, void *data) {
    scene *s = (scene *)data;
    fight_stats *fight_stats = &s->gs->fight_stats;
    arena_local *arena = (arena_local *)scene_get_userdata(s);
    chr_score *score = game_player_get_score(game_state_get_player(s->gs, event.player_id));
    object *obj_har1 = game_state_find_object(
        s->gs, game_player_get_har_obj_id(game_state_get_player(s->gs, event.player_id)));
    har *har1 = (har *)obj_har1->userdata;
    switch(event.type) {
        case HAR_EVENT_WALK:
        ...
        case HAR_EVENT_TAKE_HIT_PROJECTILE:
            arena_har_take_hit_hook(event.player_id, event.move, s);
            break;
        case HAR_EVENT_HIT_WALL:
            arena_har_hit_wall_hook(event.player_id, event.wall, s);
            break;
        ...
        case HAR_EVENT_RECOVER:
            arena_har_recover_hook(event.player_id, s);
            if(!object_is_airborne(obj_har1)) {
        ...
        case HAR_EVENT_DEFEAT:
            if(arena->state != ARENA_STATE_ENDING) {
                ...
                arena->state = ARENA_STATE_ENDING;
                arena_har_defeat_hook(event.player_id, s);
            }
            break;
        case HAR_EVENT_SCRAP:
            s->gs->fight_stats.finish = FINISH_SCRAP;
            chr_score_scrap(score);
            break;
        case HAR_EVENT_DESTRUCTION:
            s->gs->fight_stats.finish = FINISH_DESTRUCTION;
...
void maybe_install_har_hooks(scene *scene) {
    ...
    har *har1, *har2;
    har1 = (har *)obj_har1->userdata;
    har2 = (har *)obj_har2->userdata;
...
uint32_t arena_state_hash(game_state *gs) {
    ...
    for(int i = 0; i < 2; i++) {
        ...
        har *h = (har *)obj_har->userdata;
        vec2i pos = object_get_pos(obj_har);
        vec2f vel = object_get_vel(obj_har);
        uint32_t x = (uint32_t)pos.x;
        uint32_t y = (uint32_t)pos.y;
        uint32_t health = (uint32_t)h->health;
        uint32_t endurance = (uint32_t)h->endurance;
        hash = ((hash << 5) + hash) + h->id;
        ...
        hash = ((hash << 5) + hash) + h->state;
        hash = ((hash << 5) + hash) + h->executing_move;
...
void arena_state_dump(game_state *gs, char *buf, size_t bufsize) {
    ...
    for(int i = 0; i < 2; i++) {
        ...
        off = snprintf(buf + off, bufsize - off,
                       ...
                       i, player->pilot->power, player->pilot->agility, player->pilot->endurance, h->id, pos.x, pos.y,
                       h->health, h->endurance, vel.x, vel.y, state_name(h->state), h->executing_move,
...
void write_rec_move(scene *scene, game_player *player, int action) {
    ...
    if(action & ACT_PUNCH) {
        move.action = (sd_action)((int)move.action | (int)SD_ACT_PUNCH);
    }

    if(action & ACT_KICK) {
        move.action = (sd_action)((int)move.action | (int)SD_ACT_KICK);
    }

    if(action & ACT_UP) {
        move.action = (sd_action)((int)move.action | (int)SD_ACT_UP);
    }

    if(action & ACT_DOWN) {
        move.action = (sd_action)((int)move.action | (int)SD_ACT_DOWN);
    }

    if(action & ACT_LEFT) {
        move.action = (sd_action)((int)move.action | (int)SD_ACT_LEFT);
    }

    if(action & ACT_RIGHT) {
        move.action = (sd_action)((int)move.action | (int)SD_ACT_RIGHT);
    }
            ...
            if(random_int(&scene->gs->rand, info->probability) == 1) {
                ...
                if(game_state_add_object(scene->gs, obj, RENDER_LAYER_BOTTOM, 1, 0) == 0) {
                    ...
                    if(info->ani.extra_string_count > 0) {
                        ...
                        if(r > 0) {
                            str *s = (str *)vector_get(&info->ani.extra_strings, r);
...
static debug_data *create_debug_data(scene *scene) {
    ...
    debug_data *data = (debug_data *)calloc(1, sizeof(debug_data));
...
void push_players(scene *scene, game_player *p1, game_player *p2) {
    ...
    har *h1 = (har *)obj_p1->userdata;
    har *h2 = (har *)obj_p2->userdata;
        ...
        if(local->rein_enabled) {
            if(rand_float() > 0.65f) {
                ...
                for(int harnum = 0; harnum < game_state_num_players(gs); harnum++) {
                    ...
                    har *h = (har *)object_get_userdata(h_obj);
                    ...
                    // Create the object
                    object *scrap = (object *)calloc(1, sizeof(object));
...
void arena_clone(scene *src, scene *dst) {
    arena_local *local = (arena_local *)calloc(1, sizeof(arena_local));
    dst->userdata = local;
...
int arena_create(scene *scene) {
    ...
    local = (arena_local *)omf_calloc(1, sizeof(arena_local));
    scene_set_userdata(scene, local);
    ...
    for(int i = 0; i < 2; i++) {
        ...
        if(player->pilot->photo) {
            ...
            object *portrait = (object *)calloc(1, sizeof(object));
            if(i == 0) {
            ...
            for(int j = 0; j < 4; j++) {
                if(j < ceilf(local->rounds / 2.0f)) {
                    object *round_token = (object *)calloc(1, sizeof(object));
    ...
    if(scene->gs->init_flags->playback == 0) {
        if(!scene->gs->rec) {
            scene->gs->rec = (sd_rec_file *)omf_calloc(1, sizeof(sd_rec_file));
        } else {
