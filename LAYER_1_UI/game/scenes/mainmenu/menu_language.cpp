...
void menu_language_done(component *c, void *u) {
    language_menu_data *local = (language_menu_data *)menu_get_userdata(c->parent);
...
void menu_language_free(component *c) {
    language_menu_data *local = (language_menu_data *)menu_get_userdata(c);
...
void menu_language_submenu_done(component *c, component *submenu) {
    menu *m = (menu *)sizer_get_obj(c);
    m->finished = 1;
}

component *menu_language_create(scene *s) {
    // Menu userdata
    language_menu_data *local = (language_menu_data *)calloc(1, sizeof(language_menu_data));
    ...
    local->language_filenames = (char **)omf_malloc(list_size(&dirlist) * sizeof(char *));
    local->language_names = (char **)omf_malloc(list_size(&dirlist) * sizeof(char *));
    ...
    foreach(it, filename) {
        ...
        list_node *now = (list_node *)it.vnow;
        local->language_filenames[id] = (char *)now->data;
        now->data = NULL;
    }
    list_free(&dirlist);
