...
void menu_keyboard_free(component *c) {
    keyboard_menu_local *local = (keyboard_menu_local *)menu_get_userdata(c);
    omf_free(local);
...
void menu_keyboard_done(component *c, void *userdata) {
    ...
    // Apply config
    keyboard_menu_local *local = (keyboard_menu_local *)menu_get_userdata(c->parent);
    settings_keyboard *k = &settings_get()->keys;
...
gui_frame *menu_keyboard_create(scene *s, const gui_theme *theme, int selected_player) {
    keyboard_menu_local *local = (keyboard_menu_local *)calloc(1, sizeof(keyboard_menu_local));
