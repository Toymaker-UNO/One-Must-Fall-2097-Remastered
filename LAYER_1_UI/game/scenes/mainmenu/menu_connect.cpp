...
void menu_connect_free(component *c) {
    connect_menu_data *local = (connect_menu_data *)menu_get_userdata(c);
...
void menu_connect_start(component *c, void *userdata) {
    ...
    connect_menu_data *local = (connect_menu_data *)menu_get_userdata(c->parent);
    ENetAddress address;
...
void menu_connect_tick(component *c) {
    ...
    if(local->host) {
        ...
        while(!local->controllers_created && enet_host_service(local->host, &event, 0) > 0) {
            ...
            player1_ctrl = (controller *)omf_calloc(1, sizeof(controller));
            controller_init(player1_ctrl, gs);
            player1_ctrl->har_obj_id = p1->har_obj_id;
            player2_ctrl = (controller *)omf_calloc(1, sizeof(controller));
...
component *menu_connect_create(scene *s) {
    connect_menu_data *local = (connect_menu_data *)calloc(1, sizeof(connect_menu_data));
    local->s = s;
