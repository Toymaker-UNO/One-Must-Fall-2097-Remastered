...
void newsroom_free(scene *scene) {
    newsroom_local *local = (newsroom_local *)scene_get_userdata(scene);
...
void newsroom_accept_challenge_dialog_clicked(dialog *dlg, dialog_result result) {
    scene *sc = (scene *)dlg->userdata;
    newsroom_local *local = (newsroom_local *)scene_get_userdata(sc);
    game_player *p2 = game_state_get_player(sc->gs, 1);
    if(result == DIALOG_RESULT_NO) {
        p2->pilot = NULL;
...
int newsroom_create(scene *scene) {
    newsroom_local *local = (newsroom_local *)calloc(1, sizeof(newsroom_local));
