    ...
    switch(ass->op) {
        ...
        case OP_SET: {
            ...
            switch(ass->operand1.value.attr.attribute) {
                ...
                case ATTR_STAMINA:
                    // scale endurance by a magic number so it fits in a 16 bit integer
                    h->endurance = min2(operand2 * 256, h->endurance_max);
                    return true;
                default:
...
void game_state_match_settings_reset(game_state *gs) {
    ...
    gs->match_settings.knock_down = (knock_down_mode)settings_get()->advanced.knock_down;
...
int game_state_create(game_state *gs, engine_init_flags *init_flags) {
    ...
    gs->hide_ui = false;
    gs->menu_ctrl = (controller *)omf_calloc(1, sizeof(controller));
    controller_init(gs->menu_ctrl, gs);

    gs->sc = (scene *)omf_calloc(1, sizeof(scene));

    // Set up players
    for(int i = 0; i < 2; i++) {
        gs->players[i] = (game_player *)omf_calloc(1, sizeof(game_player));
    ...
    if(strlen(init_flags->rec_file) > 0 && init_flags->playback == 1) {
        gs->rec = (sd_rec_file *)omf_malloc(sizeof(sd_rec_file));
        ...
        gs->match_settings.knock_down = (knock_down_mode)gs->rec->knock_down;
...
void game_state_call_tick(game_state *gs, int mode) {
    ...
    while((s = (playing_sound *)iter_next(&it)) != NULL) {
        if(mode == TICK_DYNAMIC) {
...
void game_state_merge_sounds(game_state *old, game_state *new_state) {
    ...
    while((s = (playing_sound *)iter_next(&it)) != NULL) {
        bool found = false;
        vector_iter_begin(&new_state->sounds, &it2);
        while((s2 = (playing_sound *)iter_next(&it2)) != NULL) {
    ...
    vector_iter_begin(&new_state->sounds, &it);
    while((s = (playing_sound *)iter_next(&it)) != NULL) {
        bool found = false;
...
void game_state_static_tick(game_state *gs, bool replay) {
    ...
    if(gs->new_state) {
        // merge the sounds
        game_state_merge_sounds(gs, (game_state *)gs->new_state);
        gs = (game_state *)gs->new_state;
...
void _setup_keyboard(game_state *gs, int player_id, int control_id) {
    ...
    // Set up controller
    controller *ctrl = (controller *)calloc(1, sizeof(controller));
    game_player *player = game_state_get_player(gs, player_id);
    controller_init(ctrl, gs);

    // Set up keyboards
    keyboard_keys *keys = (keyboard_keys *)calloc(1, sizeof(keyboard_keys));
    if(control_id == 0) {
...
int render_obj_clone(render_obj *src, render_obj *dst, game_state *gs) {
    ...
    dst->obj = (object *)omf_calloc(1, sizeof(object));
...
bool game_state_hars_are_alive(game_state *gs) {
    har *h1 = (har *)object_get_userdata(game_state_find_object(gs, game_state_get_player(gs, 0)->har_obj_id));
    har *h2 = (har *)object_get_userdata(game_state_find_object(gs, game_state_get_player(gs, 1)->har_obj_id));
