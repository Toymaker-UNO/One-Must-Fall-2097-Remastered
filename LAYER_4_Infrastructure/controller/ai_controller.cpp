...
int get_enemy_range(const controller *ctrl) {
    ...
    har *h = (har *)object_get_userdata(o);
    object *o_enemy =
...
bool enemy_is_stunned_or_stasis(const controller *ctrl) {
    ...
    har *h_enemy = (har *)object_get_userdata(o_enemy);
...
int ai_controller_poll(controller *ctrl, ctrl_event **ev) {
    ai *a = (ai *)ctrl->data;
    object *o = game_state_find_object(ctrl->gs, ctrl->har_obj_id);
    scene *s = game_state_get_scene(ctrl->gs);
    if(s->id == SCENE_VS || s->id == SCENE_NEWSROOM) {
        if(ctrl->gs->warp_speed || (s->static_ticks_since_start + 1) % 256 == 0) {
...
void ai_controller_create(controller *ctrl, int difficulty, sd_pilot *pilot, int pilot_id) {
    ...
    tactic_state *tactic = (tactic_state *)calloc(1, sizeof(tactic_state));
