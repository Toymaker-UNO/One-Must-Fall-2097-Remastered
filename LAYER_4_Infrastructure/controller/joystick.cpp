...
static int internal_joystick_poll(joystick *k, controller *ctrl, ctrl_event **ev, bool allow_esc) {
    ...
    Sint16 x_axis = SDL_GameControllerGetAxis(k->joy, (SDL_GameControllerAxis)k->keys->x_axis);
    Sint16 y_axis = SDL_GameControllerGetAxis(k->joy, (SDL_GameControllerAxis)k->keys->y_axis);
    int dpadup = SDL_GameControllerGetButton(k->joy, (SDL_GameControllerButton)k->keys->dpad[0]);
    int dpaddown = SDL_GameControllerGetButton(k->joy, (SDL_GameControllerButton)k->keys->dpad[1]);
    int dpadleft = SDL_GameControllerGetButton(k->joy, (SDL_GameControllerButton)k->keys->dpad[2]);
    int dpadright = SDL_GameControllerGetButton(k->joy, (SDL_GameControllerButton)k->keys->dpad[3]);
    ...
    // button input
    if(SDL_GameControllerGetButton(k->joy, (SDL_GameControllerButton)k->keys->punch)) {
        action |= ACT_PUNCH;
    } else if(SDL_GameControllerGetButton(k->joy, (SDL_GameControllerButton)k->keys->kick)) {
        action |= ACT_KICK;
    }
    ...
    if(allow_esc && SDL_GameControllerGetButton(k->joy, (SDL_GameControllerButton)k->keys->escape)) {
...
int joystick_create(controller *ctrl, int joystick_id) {
    joystick *k = (joystick *)calloc(1, sizeof(joystick));
    k->keys = (joystick_keys *)omf_calloc(1, sizeof(joystick_keys));
