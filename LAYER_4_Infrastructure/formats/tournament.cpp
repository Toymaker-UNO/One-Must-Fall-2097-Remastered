...
int sd_tournament_set_pic_name(sd_tournament_file *trn, const char *pic_name) {
    ...
    trn->pic_file = (char *)omf_realloc(trn->pic_file, len);
...
int sd_tournament_load(sd_tournament_file *trn, const char *filename) {
    ...
    uint16_t enemy_count, unknown_b;
    char *justfile;
    int victory_text_offset;
    ...
    enemy_count = sd_read_uword(r);
    if(enemy_count >= MAX_TRN_ENEMIES || enemy_count == 0) {
        goto error_0;
    }
    unknown_b = sd_read_uword(r);

    justfile = strrchr(filename, pm_path_sep);
    if(justfile == NULL) {
    ...
    victory_text_offset = sd_read_dword(r);
    ...
    for(unsigned i = 0; i < trn->enemy_count; i++) {
        trn->enemies[i] = (sd_pilot *)omf_calloc(1, sizeof(sd_pilot));
