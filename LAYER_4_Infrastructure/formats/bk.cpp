...
int sd_bk_copy(sd_bk_file *dst, const sd_bk_file *src) {
    ...
    for(int i = 0; i < MAX_BK_ANIMS; i++) {
        if(src->anims[i] != NULL) {
            dst->anims[i] = (sd_bk_anim *)omf_calloc(1, sizeof(sd_bk_anim));
    ...
    if(src->background != NULL) {
        dst->background = (sd_vga_image *)omf_calloc(1, sizeof(sd_vga_image));
    ...
    for(int i = 0; i < MAX_BK_PALETTES; i++) {
        ...
        if(src->palettes[i] != NULL) {
            dst->palettes[i] = (vga_palette *)omf_calloc(1, sizeof(vga_palette));
            dst->remaps[i] = (vga_remap_tables *)omf_calloc(1, sizeof(vga_remap_tables));
...
int sd_bk_load(sd_bk_file *bk, const char *filename) {
    ...
    while(1) {
        ...
        bk->anims[animno] = (sd_bk_anim *)omf_calloc(1, sizeof(sd_bk_anim));
    ...
    bk->background = (sd_vga_image *)omf_calloc(1, sizeof(sd_vga_image));
    ...
    for(uint8_t i = 0; i < bk->palette_count; i++) {
        bk->palettes[i] = (vga_palette *)omf_malloc(sizeof(vga_palette));
        if((ret = palette_load(r, bk->palettes[i])) != SD_SUCCESS) {
            goto exit_0;
        }
        bk->remaps[i] = (vga_remap_tables *)omf_malloc(sizeof(vga_remap_tables));
...
int sd_bk_load_from_pcx(sd_bk_file *bk, const char *filename) {
    int ret;
    pcx_file *pcx = (pcx_file *)calloc(1, sizeof(pcx_file));
    ...
    bk->palettes[0] = (vga_palette *)omf_malloc(sizeof(vga_palette));
    bk->remaps[0] = (vga_remap_tables *)omf_malloc(sizeof(vga_remap_tables));
...
int sd_bk_set_anim(sd_bk_file *bk, int index, const sd_bk_anim *anim) {
    ...
    bk->anims[index] = (sd_bk_anim *)omf_calloc(1, sizeof(sd_bk_anim));
...
int sd_bk_set_palette(sd_bk_file *bk, int index, const vga_palette *pal) {
    ...
    if(bk->palettes[index] == NULL) {
        bk->palettes[index] = (vga_palette *)omf_malloc(sizeof(vga_palette));
    }
...
int sd_bk_push_palette(sd_bk_file *bk, const vga_palette *pal) {
    ...
    bk->palettes[bk->palette_count] = (vga_palette *)omf_malloc(sizeof(vga_palette));
