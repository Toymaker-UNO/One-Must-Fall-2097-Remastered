# Incremental Build Script (no clean) - same as 00_build.ps1 but keeps existing .o and only recompiles when needed
Write-Host "=== C++ Incremental Build (no clean) ===" -ForegroundColor Magenta
...
$CFLAGS += "-fpermissive"
$CFLAGS += "-include"; $CFLAGS += "stdint.h"
$CFLAGS += "-include"; $CFLAGS += "stddef.h"
$CFLAGS += "-include"; $CFLAGS += "string.h"
$CFLAGS += "-include"; $CFLAGS += "stdbool.h"
...
# Build dir: do NOT clean; create only if missing
if (!(Test-Path $BUILD_DIR)) {
    Write-Host "Creating build directory..." -ForegroundColor Yellow
    New-Item -ItemType Directory -Path $BUILD_DIR | Out-Null
} else {
    Write-Host "Using existing build directory (incremental)." -ForegroundColor Yellow
}
...
# Compile (incremental: only when .o missing or source newer than .o)
Write-Host "Compiling (incremental)..." -ForegroundColor Yellow
$OBJECT_FILES = @()

function NeedsCompile($srcPath, $objPath) {
    if (!(Test-Path $objPath)) { return $true }
...
foreach ($source_file in $C_FILES) {
    ...
    if (NeedsCompile $source_file $object_file) {
...
if (Test-Path $GAME_RESOURCES_DIR) {
    ...
    foreach ($file in $RESOURCE_FILES) {
        try {
            ...
            $COPIED_COUNT++
        } catch { Write-Host "Failed to copy: $($file.Name)" -ForegroundColor Red }
    }
...
if (Test-Path $SHADERS_DIR) {
    ...
    foreach ($file in $SHADER_FILES) {
        try {
            ...
            $COPIED_COUNT++
        } catch { Write-Host "Failed to copy shader: $($file.Name)" -ForegroundColor Red }
    }
...
Write-Host "=== Incremental Build Completed ===" -ForegroundColor Magenta
