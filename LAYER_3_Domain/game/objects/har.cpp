...
void har_free(object *obj) {
    har *h = (har *)object_get_userdata(obj);
    ...
    foreach(it, pair) {
        animation *anim = (animation *)pair->value;
        animation_free(anim);
    }
...
void har_stunned_done(object *har_obj) {
    har *h = (har *)object_get_userdata(har_obj);
...
char get_last_input(object *obj) {
    har *h = (har *)object_get_userdata(obj);
    return flip_input(h->inputs[0], object_get_direction(obj));
}
...
void cb_har_disable_animation(object *parent, uint8_t animation_id, uint16_t ticks, void *userdata) {
    object *har_obj = (object *)userdata;
    har *h = (har *)object_get_userdata(har_obj);
...
void har_move(object *obj) {
    ...
    object *enemy_obj =
        game_state_find_object(obj->gs, game_player_get_har_obj_id(game_state_get_player(obj->gs, !h->player_id)));
    har *enemy_har = (har *)object_get_userdata(enemy_obj);
...
bool har_find_linked_objects_pred(const object *obj, void *data) {
    struct linked_object_query *q = (struct linked_object_query *)data;
...
int is_destruction(game_state *gs) {
    har *har_a = (har *)object_get_userdata(game_state_find_object(gs, game_state_get_player(gs, 0)->har_obj_id));
    har *har_b = (har *)object_get_userdata(game_state_find_object(gs, game_state_get_player(gs, 1)->har_obj_id));
...
void har_block(object *obj, vec2i hit_coord, uint8_t block_stun) {
    ...
    object *scrape = (object *)calloc(1, sizeof(object));
...
void har_check_closeness(object *obj_a, object *obj_b) {
    ...
    har *a = (har *)object_get_userdata(obj_a);
    har *b = (har *)object_get_userdata(obj_b);
...
void har_collide_with_projectile(object *o_har, object *o_pjt) {
    ...
    har *other = (har *)object_get_userdata(
...
void har_collide_with_hazard(object *o_har, object *o_hzd) {
    har *h = (har *)object_get_userdata(o_har);
    bk *bk_data = (bk *)object_get_userdata(o_hzd);
    ...
    if(!h->damage_received && intersect_har_sprite_hitpoint(o_hzd, o_har, level, &hit_coord)) {
        ...
        har *enemy_h = (har *)object_get_userdata(enemy_obj);
...
static void har_palette_transform(damage_tracker *damage, vga_palette *pal, void *userdata) {
    ...
    const har *h = (har *)object_get_userdata(obj);
...
void har_tick(object *obj) {
    ...
    if(hashmap_size(&h->disabled_animations)) {
        ...
        hashmap_pair *pair = NULL;
        foreach(it, pair) {
            uint16_t *value = (uint16_t *)pair->value;
            if(*value <= 1) {
    ...
    if(object_has_effect(obj, EFFECT_TRAIL) && obj->age % 2 == 0 &&
       (cur_sprite = animation_get_sprite(obj->cur_animation, obj->cur_sprite_id))) {
        ...
        object *nobj = (object *)calloc(1, sizeof(object));
...
void har_finished(object *obj) {
    ...
    har *har_enemy = (har *)object_get_userdata(obj_enemy);
...
int har_clone(object *src, object *dst) {
    har *oldlocal = (har *)object_get_userdata(src);
    har *local = (har *)calloc(1, sizeof(har));
...
int har_create(object *obj, af *af_data, int dir, int har_id, int pilot_id, int player_id) {
    ...
    local->trail_cache = (hashmap *)omf_calloc(1, sizeof(hashmap));
        ...
        if(move != NULL) {
            ...
            if(move->ani.extra_string_count > 0 && move->extra_string_selector != 1 &&
               move->extra_string_selector != 2) {
                ...
                str *st = (str *)vector_get(&move->ani.extra_strings, extra_index);
                if(st && str_size(st) != 0 && !str_equal_c(st, "!")) {
                    // its not the empty string and its not the string '!'
                    // so we should use it
                    str_set(&move->ani.animation_string, st);
                    if(pilot->enhancements[har_id] > 0) {
                        log_debug("using enhancement %d string '%s' for animation %d on har %d",
                                  pilot->enhancements[har_id], str_c(st), i, har_id);
                    } else {
                        log_debug("using %s mode string '%s' for animation %d on har %d",
                                  fight_mode ? "hyper" : "normal", str_c(st), i, har_id);
                    }
                }
            }
                    ...
                    case 1:
                        ...
                        if(move->ani.extra_string_count > 0) {
                            ...
                            str_set(&move->ani.animation_string,
                                    (const str *)vector_get(&move->ani.extra_strings,
